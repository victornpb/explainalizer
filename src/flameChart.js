...
class FlameChart extends EventEmitter {
    ...
    renderChart() {
        ...
        walk(this.data, (node, level) => {
            ...
            if (x + w > 0
                ...
                this.renderRect(this.getColor(type), node.textColor || 'black', name, x, y, w);
    ...
    renderRect(color, textColor='black', text, x, y, w) {
        this.ctx.fillStyle = color;
    ...
    renderTooltip() {
        if (this.hoveredRegion) {
            const node = this.hoveredRegion.node;
            ...
            const nodeColor = this.getColor(node.type);
            
            const fragments = [
                { text: `${node.name}`, color: '#ffffff' },
                '\n',
                { text: `${duration.toFixed(2)} ms`, color: '#18FFFF' }, 
                { text: ``, color: 'silver' }, 
            ]
            if (children && children.length) fragments.push(
                '\n',
                { text: `${selfTime.toFixed(2)} ms`, color: '#EEFF41' },
                { text: ` (Self)`, color: 'silver' },
            )
            ...
            for (let f of fragments){
                if(f==='\n') lines[lines.length] = '';
                else lines[lines.length-1] += f.text;
            }
            const sizes = lines.map(lineText => this.ctx.measureText(lineText));
            const textBlockWidth = Math.max(...sizes.map(m=>m.width));
            const textBlockHeight = sizes.reduce((sum=0, m)=> sum + lineSpacing + m.fontBoundingBoxAscent + m.fontBoundingBoxDescent, 0);
            
            // Draw tooltip box
            const boxWidth = textBlockWidth + (this.blockPadding * 2);
            const boxHeight = textBlockHeight + (this.blockPadding * 2);
            ...
            drawCanvasText(
                this.ctx,
                tooltipX + this.blockPadding,
                tooltipY + this.blockPadding,
                fragments,
...
function drawCanvasText(ctx, x, y, textArray, {lineSpacing}) {
    let caretX = 0;
    let caretY = 0;
    let lineY = 0;
    
    for(const fragment of textArray) {
        if(fragment!=='\n'){
            if(fragment.color) ctx.fillStyle = fragment.color;
            const m = ctx.measureText(fragment.text);
            lineY = m.fontBoundingBoxAscent + (lineSpacing/2);
            ctx.fillText(
                fragment.text,
                x + caretX,
                y + caretY + lineY, //TODO: fix text baseline
                );
            caretX += m.width;
        }
        else {
            caretY += lineY + (lineSpacing/2); //TODO: fix text baseline
            caretX = 0;
        }
    }
}
